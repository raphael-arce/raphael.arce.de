...
<details className="group collapse collapse-plus bg-gray-200 hover:bg-gray-300">
  ...
  <div className="xs:w-auto 2xs:w-96 collapse-content mx-auto w-72">
    <Example1Simplified />
  </div>
</details>
...
The state is declared in the component <code className="bg-gray-200">BranchA</code>.
The state's value is passed down as a prop to <code className="bg-gray-200">LeaveAA</code>
and the setter function to <code className="bg-gray-200">LeaveAB</code>.
But if you click on the `X*2` button you'll notice that not only the component <code className="bg-gray-200">LeaveAA</code>,
...
That means that only the <code className="bg-gray-200">LeaveBA</code> receives the state's value,
while the <code className="bg-gray-200">LeaveBB</code> receives the setter function.
Big difference! Thanks to that, only the component <code className="bg-gray-200">LeaveBA</code> is
...
So we lifted the state up once further, to the <code className="bg-gray-200">Root</code> component, and prop-drilled our
value/setter down to the leaves. And now the difference: when you click on <code className="bg-gray-200">X\*2</code>,
ALL components are re-rendered. But when you click on <code className="bg-gray-200">Y\*2</code>, still
only the <code className="bg-gray-200">LeaveBA</code> is re-rendered.
